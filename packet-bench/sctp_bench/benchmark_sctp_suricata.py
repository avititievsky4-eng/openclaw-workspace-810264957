...
def main():
    ...
    pcap=f"/tmp/sctp_suricata_{int(time.time()*1000)}.pcap"
    ...
    outdir=tempfile.mkdtemp(prefix='suri_sctp_')
    ...
    if os.path.exists(eve):
        with open(eve,errors='ignore') as f:
            for ln in f:
                if '"proto":"SCTP"' in ln or '"proto": "SCTP"' in ln:
                    captured += 1
    try: os.remove(pcap)
    except: pass
    try:
    ...
    print(json.dumps({'tool':'suricata-sctp','sent':sent,'captured':captured,'captured_normalized':norm,'capture_ratio':(captured/sent if sent else 0.0),'capture_ratio_normalized':(norm/sent if sent else 0.0)},indent=2))
