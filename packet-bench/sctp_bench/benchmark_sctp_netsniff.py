...
def main():
    ...
    # netsniff-ng capture to pcap for duration window
    pcap=f"/tmp/sctp_netsniff_{int(time.time()*1000)}.pcap"
    cap=subprocess.Popen(['netsniff-ng','--in',args.iface,'--out',pcap,'--silent'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,text=True)
    time.sleep(0.3)
    ...
    # parse captured pcap via tcpdump for SCTP lines
    parse=subprocess.run(['tcpdump','-r',pcap,'-n','sctp'],capture_output=True,text=True)
    captured=len([ln for ln in (parse.stdout or '').splitlines() if ln.strip()])
    try: os.remove(pcap)
    except: pass

    norm=captured//2 if args.iface=='lo' else captured
    print(json.dumps({'tool':'netsniff-sctp','sent':sent,'captured':captured,'captured_normalized':norm,'capture_ratio':(captured/sent if sent else 0.0),'capture_ratio_normalized':(norm/sent if sent else 0.0)},indent=2))
