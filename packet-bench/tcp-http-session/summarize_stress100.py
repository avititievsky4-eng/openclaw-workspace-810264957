...
def main():
    ...
    for fp in files:
        ...
        for sid in checked:
            rec = sess[str(sid)]
            if rec.get('min2_asset_ok', rec.get('min1_asset_ok', rec.get('min20_ok', False))):
                success += 1
        ...
        rows.append({
            ...
            'tcp_sessions_success_min2asset_min1asset': success,
    ...
    with open(out_csv, 'w', newline='') as f:
        w = csv.DictWriter(
            f,
            fieldnames=['method', 'sessions_detected', 'sessions_checked', 'tcp_sessions_success_min2asset_min1asset', 'success_rate', 'notes'],
        )
        w.writeheader()
        w.writerows(rows)
    ...
    with open(out_md, 'w') as f:
        ...
        f.write('| Method | sessions_detected | sessions_checked | tcp_sessions_success_min2asset | success_rate | Notes |\n')
        f.write('|---|---:|---:|---:|---:|---|\n')
        for r in rows:
            f.write(
                f"| {r['method']} | {r['sessions_detected']} | {r['sessions_checked']} | {r['tcp_sessions_success_min2asset_min1asset']} | {r['success_rate']:.2f}% | {r['notes']} |\n"
            )
    ...
    for r in rows:
        ...
        with open(method_md, 'w') as f:
            ...
            f.write(f"- tcp_sessions_success_min2asset: {r['tcp_sessions_success_min2asset_min1asset']}\n")
