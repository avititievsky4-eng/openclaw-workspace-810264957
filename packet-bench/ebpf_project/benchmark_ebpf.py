...
def main():
    ...
    # eBPF tracepoint counter (interface-level, not UDP-port specific)
    program = (
        f'tracepoint:net:netif_receive_skb /str(args->name) == "{args.iface}"/ '
        '{ @pkts = count(); }'
    )
    ...
    result = {
        ...
        "elapsed_s": t1 - t0,
        "note": "Counts interface receive events on lo via eBPF tracepoint (not UDP-port filtered).",
    }
